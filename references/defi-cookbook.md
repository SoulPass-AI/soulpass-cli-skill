...
### Mint Addresses Work Everywhere

All token commands (`swap`, `pay`, `approve`, `lend`, `balance`, `token-info`, `sign x402`) accept either a known symbol (SOL, USDC) or a raw mint address via `--token`. Decimals are auto-resolved from on-chain, so `--amount` is always human-readable:

```bash
# By symbol
soulpass swap --from SOL --to USDC --amount 1.5

# By mint address — same human-readable amount
soulpass swap --from So11111111111111111111111111111111111111112 \
  --to EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v --amount 1.5
```

Token-2022 mints are detected automatically — no extra flags needed.
...
# Step 2: Check on-chain security (mint authority, freeze authority)
...
- `mintAuthority` is NOT null (check via `token-info`) — project can mint unlimited tokens and dilute your holdings
...
- `mintAuthority: null` in `token-info` — mint authority revoked, supply is fixed
- `freezeAuthority: null` — no one can freeze your tokens
- Liquidity locked

**Safe entry pattern:**

```bash
# 1. Risk check — price + on-chain security
soulpass price BONK
